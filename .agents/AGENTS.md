...
8.  **Transferencias en Dashboard:** En el selector, las transferencias recibidas se clasifican bajo el tipo verde `Ingreso` y las propias salientes bajo `Egreso > Transferencias Propias`; no ofrecer `Movimiento Interno` como una taxonomía paralela. En cálculos, una transferencia propia recibida forma parte de las entradas disponibles y del saldo tras cobertura, mientras que una transferencia propia saliente permanece visible pero no se descuenta como gasto real. Las categorías legadas `Movimiento Interno` siguen siendo reconocidas por los reportes.
9.  **Vista Light y transferencias:** Debe aplicar la misma semántica del Dashboard principal. Separar ingresos reales de transferencias recibidas, sumar ambos como entradas disponibles, excluir transferencias propias enviadas de gastos y presentar un balance negativo como `Uso de saldo previo estimado`.
