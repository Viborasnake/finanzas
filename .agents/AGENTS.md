...
8.  **Transferencias en Dashboard:** En el selector, las transferencias recibidas se clasifican bajo el tipo verde `Ingreso` y las propias salientes bajo `Egreso > Transferencias Propias`; no ofrecer `Movimiento Interno` como una taxonomía paralela. En cálculos de producto, una transferencia propia recibida suma a ingresos y entradas disponibles, mientras que una transferencia propia saliente permanece visible pero no se descuenta como gasto. La distinción patrimonial se conserva internamente y las categorías legadas `Movimiento Interno` siguen siendo reconocidas.
9.  **Vista Light y transferencias:** Debe aplicar la misma semántica del Dashboard principal. Incluir las transferencias propias recibidas dentro del total de ingresos, identificarlas en el desglose, excluir transferencias propias enviadas de gastos y presentar el saldo estimado al cierre con su apertura automática.
10. **Saldo inicial por periodo:** Derivarlo automáticamente del último saldo bancario reportado antes del inicio del periodo. El flujo neto siempre usa exclusivamente las fechas seleccionadas. Para explicar la posición del usuario, mostrar `saldo inicial + entradas - salidas = saldo estimado al cierre`; el saldo inicial da continuidad recursiva entre meses, pero no altera el flujo neto aislado. Priorizar el saldo estimado al cierre por sobre un déficit económico aislado y dejar ese análisis como información secundaria. No saturar la tarjeta principal con explicaciones de cobertura, bancos faltantes o ajustes; el resultado del mes debe expresarse en una frase y una operación simple.
11. **DAP e inversiones:** La constitución y el rescate del capital son movimientos patrimoniales neutros: no suman ingresos ni gastos. Detectar liquidaciones y captaciones también por descripción para cubrir clasificaciones históricas incorrectas. Solo una rentabilidad informada por separado se contabiliza como ingreso.
12. **Navegación mensual:** Los selectores principales del Resumen financiero y Vista Light deben compartir el control segmentado con flechas anterior/siguiente y cambiar un mes calendario completo. No permitir avanzar más allá del mes actual; el texto central conserva el selector de periodo correspondiente.
13. **Pago de tarjeta:** Un movimiento clasificado como `Pago Tarjeta Crédito` liquida deuda generada por consumos anteriores. Si el usuario no ha importado compras individuales de tarjeta, este pago se considerará directamente como el consumo del mes (gasto real) por defecto. Documentar que existirá un desfase natural de ~1 mes entre la ocurrencia del gasto real y el pago facturado, lo cual es el comportamiento esperado.
14. **Revisiones financieras accionables:** No agrupar problemas contables diferentes bajo una sola instrucción. Cada revisión debe explicar si es opcional, qué cifra afecta, qué pasa si se omite y ofrecer una acción directa al movimiento o importador correspondiente.
14. **Doble lectura financiera:** Todos los reportes deben distinguir `flujo de caja` (todo abono/cargo bancario) de `resultado económico` (ingresos menos consumo). Nunca ocultar una salida de caja solo porque sea patrimonial o reduzca deuda.
15. **Cobertura de tarjeta:** Por defecto, una cobertura ausente no generará exclusiones ni advertencias visibles, contabilizando el pago de la tarjeta como consumo directo. La lógica de evaluación interna se conserva solo para soportar conciliación en escenarios donde sí existan compras importadas.
16. **Cuotas de créditos:** Una cuota sin desglose permanece completa como salida de caja y, conservadoramente, como gasto con advertencia. Para corregirla, dividirla en `Servicio de Deuda > Capital de Crédito` (reduce pasivo, no consumo), `Intereses de Crédito` y opcionalmente `Seguros y Comisiones` (ambos gasto real). Nunca estimar el reparto sin datos del banco.
17. **Servicios facturados:** Luz, agua, gas, planes y servicios similares cuentan por fecha de pago mientras no exista un periodo de servicio explícito. No tratarlos como liquidación duplicada.
