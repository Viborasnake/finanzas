      ...
      >
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          ...
          <div>
            ...
            <span style={{ display: 'block', color: 'var(--text-muted)', fontSize: '0.82rem', fontWeight: 800, marginTop: '0.25rem' }}>{subtitle}</span>
          </div>
        </div>
...
export default function Settings() {
  ...
  const renderSetupMiniDashboard = () => {
    ...
    return (
      <section style={{ border: '2px solid var(--border-color)', borderRadius: '12px', boxShadow: 'var(--shadow-md)', backgroundColor: 'var(--surface-color)', marginBottom: '2rem', overflow: 'hidden' }}>
        <button
          type="button"
        ...
        >
          <div style={{ minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', flexWrap: 'wrap', marginBottom: '0.45rem' }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem', padding: '0.25rem 0.6rem', border: '2px solid var(--border-color)', borderRadius: '999px', backgroundColor: 'var(--pastel-blue)', boxShadow: 'var(--shadow-sm)', fontSize: '0.72rem', fontWeight: 900 }}>
                <Activity size={14} strokeWidth={3} />
                Estado del banco
              </span>
              <strong style={{ fontSize: '1rem' }}>{dashboardBankLabel}</strong>
              <span style={{ color: 'var(--text-muted)', fontWeight: 800, fontSize: '0.85rem' }}>{doneCount}/{items.length} listo</span>
            </div>
            ...
            {items.map(item => (
              ...
              >
                ...
                <span style={{ minWidth: 0 }}>
                  ...
                  <span style={{ display: 'block', color: 'var(--text-secondary)', fontWeight: 700, fontSize: '0.76rem', lineHeight: 1.35, overflowWrap: 'anywhere' }}>
                    {item.detail}
                  </span>
                </span>
              ...
              return (
                <div key={bank.id} className="settings-list-row bank-row">
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    {isMain && (
                      <span style={{ fontSize: '0.7rem', padding: '0.2rem 0.5rem', backgroundColor: 'var(--pastel-yellow)', color: 'var(--warning-text)', borderRadius: '999px', fontWeight: 900, border: '2px solid var(--border-color)' }}>
                        BANCO PRINCIPAL
                      </span>
                    )}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    ...
                    {connectedBanks.length > 1 && (
                      <button 
                        ...
                        style={{ padding: '0.5rem', backgroundColor: 'var(--danger-surface)', color: 'var(--danger-text)', border: '2px solid var(--danger)' }}
        ...
        <CollapsibleSection id="categorias" icon={Tags} title="Mis Categorías" subtitle="Categorías personalizadas compartidas entre todos tus bancos" description="Agrega nuevas categorías para organizar tus movimientos. Estas se sumarán a la lista base que ya trae la aplicación." defaultCollapsed={true}>
          ...
          {customCategories.length > 0 ? (
            ...
            <div className="settings-empty">
              <p style={{ margin: 0, color: 'var(--text-muted)', fontWeight: 600 }}>No has creado categorías personalizadas aún.</p>
            </div>
          )}
        </CollapsibleSection>
