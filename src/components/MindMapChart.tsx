...
export default function MindMapChart({ transactions, taxonomy }: MindMapChartProps) {
  ...
  const controls = (
    <div style={{ position: 'absolute', bottom: '1rem', right: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem', zIndex: 10 }}>
      <button type="button" aria-label="Acercar mapa" onClick={() => setZoom(z => Math.min(z + 0.2, 2))} style={{ backgroundColor: 'var(--surface-color)', border: '2px solid #000', borderRadius: '8px', padding: '0.5rem', width: '44px', height: '44px', display: 'flex', justifyContent: 'center', alignItems: 'center', fontWeight: 900, cursor: 'pointer', boxShadow: '2px 2px 0px #000' }}>+</button>
      <button type="button" aria-label="Alejar mapa" onClick={() => setZoom(z => Math.max(z - 0.2, 0.2))} style={{ backgroundColor: 'var(--surface-color)', border: '2px solid #000', borderRadius: '8px', padding: '0.5rem', width: '44px', height: '44px', display: 'flex', justifyContent: 'center', alignItems: 'center', fontWeight: 900, cursor: 'pointer', boxShadow: '2px 2px 0px #000' }}>-</button>
      <button type="button" onClick={() => setHideEmpty(!hideEmpty)} title={hideEmpty ? "Mostrar sin movimientos" : "Ocultar sin movimientos"} aria-label={hideEmpty ? "Mostrar categorías sin movimientos" : "Ocultar categorías sin movimientos"} aria-pressed={hideEmpty} style={{ backgroundColor: 'var(--surface-color)', border: '2px solid #000', borderRadius: '8px', padding: '0.5rem', width: '44px', height: '44px', display: 'flex', justifyContent: 'center', alignItems: 'center', fontWeight: 900, cursor: 'pointer', boxShadow: '2px 2px 0px #000' }}>
        {hideEmpty ? <EyeOff size={20} /> : <Eye size={20} />}
      </button>
      <button type="button" aria-label={isModalOpen ? 'Cerrar mapa ampliado' : 'Ampliar mapa'} aria-pressed={isModalOpen} onClick={() => setIsModalOpen(!isModalOpen)} style={{ backgroundColor: 'var(--surface-color)', border: '2px solid #000', borderRadius: '8px', padding: '0.5rem', width: '44px', height: '44px', display: 'flex', justifyContent: 'center', alignItems: 'center', fontWeight: 900, cursor: 'pointer', boxShadow: '2px 2px 0px #000' }}>
