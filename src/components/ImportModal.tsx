...
export default function ImportModal({ onClose, presentation = 'dialog' }: ImportModalProps = {}) {
  ...
  const handleDescriptionBlur = (index: number) => {
    ...
    if (row.description !== row.original_description && row.description.trim() !== '') {
      ...
      if (othersCount > 0) {
        toast.custom((t) => (
          <div className="card" style={{ padding: '1.5rem', border: '2px solid var(--border-color)', boxShadow: 'var(--shadow-md)', background: 'var(--surface-color)', maxWidth: '400px' }}>
  ...
  const content = (
    <div className="import-flow-content">
      ...
      {error && (
        <div role="alert" style={{ backgroundColor: 'var(--danger-surface)', color: 'var(--danger-text)', padding: '1rem', borderRadius: 'var(--radius-md)', marginBottom: '1.5rem', border: '2px solid var(--border-color)', boxShadow: 'var(--shadow-md)', display: 'flex', alignItems: 'center', gap: '0.75rem', fontWeight: 700 }}>
          <AlertTriangle />
          {error}
        </div>
      )}
      ...
      {step === 'confirm' && (
        <div className="card animate-fade-in import-confirm-card" style={{ textAlign: 'center', border: '2px solid var(--border-color)', boxShadow: 'var(--shadow-md)' }}>
           ...
           <div style={{ marginBottom: '3rem' }}>
             ...
             <select
               ...
               style={{ maxWidth: '300px', margin: '0 auto', fontSize: '1.25rem', textAlign: 'center', padding: '1rem', border: '2px solid var(--border-color)', borderRadius: '12px', boxShadow: 'var(--shadow-md)', cursor: 'pointer', fontWeight: 800 }}
               value={detectedBank || ''} 
      ...
      {step === 'preview' && data.length > 0 && (
        <div className="card animate-fade-in import-preview-card">
          ...
          <div style={{ maxHeight: '400px', overflowY: 'auto', border: '2px solid var(--border-color)', borderRadius: 'var(--radius-sm)' }}>
      ...
      {showPasswordModal && (
        ...
        >
            ...
            {passwordError && (
              <div role="alert" style={{ color: 'var(--danger-text)', fontWeight: 700, marginBottom: '1rem', padding: '0.75rem', backgroundColor: 'var(--danger-surface)', borderRadius: '8px', border: '2px solid var(--border-color)', fontSize: '0.9rem' }}>
                {passwordError}
              </div>
            )}
