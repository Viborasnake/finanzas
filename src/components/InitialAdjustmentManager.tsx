              ...
              <div key={bank} style={{ border: '2px solid var(--border-color)', borderRadius: 'var(--radius-md)', overflow: 'hidden' }}>
                <div style={{ backgroundColor: adj ? 'var(--surface-subtle)' : 'var(--account-card-unconfigured)', padding: '1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    {adj ? (
                      <>
                        <button type="button" className="btn" onClick={() => isEditingThis ? setEditingBank(null) : handleEditClick(bank)} style={{ backgroundColor: 'var(--surface-color)', border: '2px solid var(--border-color)', boxShadow: 'var(--shadow-sm)' }}>
                          <Edit2 size={16} /> {isEditingThis ? 'Ocultar' : 'Editar'}
                        </button>
                        <button type="button" className="btn" onClick={() => handleDelete(bank)} style={{ backgroundColor: 'var(--danger-surface)', color: 'var(--danger-text)', border: '2px solid var(--border-color)', boxShadow: 'var(--shadow-sm)' }}>
